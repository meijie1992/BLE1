	...
	) // 主函数
{
	...
	do {
		...
		if (flag_uart2_cmd) // 接收群控器rs485数据
		{
			...
			if(uart2_receive_buffer[1]==0xf0) //caiwk-------------------------------
			{
				uart1_send_byte(uart2_receive_buffer[1]);
				
				uart1_send_byte(uart2_receive_buffer[2]);
				
				uart1_send_byte(uart2_receive_buffer[3]);
			}
			else
			{
				uart1_send_byte(uart2_receive_buffer[3]);			

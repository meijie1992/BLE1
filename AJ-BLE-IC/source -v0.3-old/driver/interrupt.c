 	...
 	) // 中断服务程序
{
	...
	if (TMR1IE && TMR1IF) // 定时器1中断 1ms间隔
	{
		...
		else if (count_uart2_receive == 0x00)
		{
			uart2_cnt = 0x00;
		}
		else
	...
	if (RC2IE && RC2IF) // uart2，rs485接收数据中断，群控器发送，读卡器接收，群控器询问有没有刷卡记录
	{
		...
		if (flag_uart2_cmd == 0x00)
		{
			...
			if (uart2_receive_buffer[0] == P_H_R) // 包头数据正确，接收计数累加
			{
				uart2_cnt++;
			}
			count_uart2_receive = 0x03; // 接收字符与字符超时时间为3ms 9600bps
			if (uart2_cnt > 0x01)
			{
				data_length = uart2_receive_buffer[1]; // 保存接收包数据总长度
				if ((uart2_cnt == data_length) && (uart2_receive_buffer[data_length - 0x01] == P_T_R))
				{
					uart2_cnt = 0x00;
